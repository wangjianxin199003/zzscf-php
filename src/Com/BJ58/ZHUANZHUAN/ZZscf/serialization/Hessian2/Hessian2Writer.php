...
use Com\BJ58\ZHUANZHUAN\ZZscf\Serialization\HessianStreamResult;
...
//                $classdef->props = ->array_keys(get_class_vars($class));
                $reflection = $this->options->objectFactory->getObjectReflectionClass($class);
                $properties = $reflection->getProperties();
                foreach ($properties as $property){
                    $classdef->props[] = $property->name;
                }
            }
        ...
        if ($this->between($value, -16, 47)) {
            ...
            if ($this->between($value, -2048, 2047)) {
                ...
                if ($this->between($value, -262144, 262143)) {
                    ...
                    $stream .= pack('c', $value);
                    return $stream;
                } else if ($this->between($value, -0x80000000, +0x7fffffff)) {
                    $stream = 'I';
    ...
    public function packInt64($value)
    {

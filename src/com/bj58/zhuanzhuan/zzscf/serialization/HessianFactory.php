...
use com\bj58\zhuanzhuan\zzscf\serialization\Hessian2\Hessian2IteratorWriter;
use com\bj58\zhuanzhuan\zzscf\serialization\Hessian2\Hessian2ServiceParser;
use com\bj58\zhuanzhuan\zzscf\serialization\Hessian2\Hessian2ServiceWriter;
use DateTime;
use Exception;
use stdClass;
...
class HessianObjectFactory implements IHessianObjectFactory
{
    ...
    public function getObject($type)
    {
        ...
        $reflection = $this->getObjectReflectionClass($type);
        return $reflection->newInstanceWithoutConstructor();
    }

    public function getObjectReflectionClass($type)
    {
        if (array_key_exists($type, $this->reflectionMap)) {
            return $this->reflectionMap[$type];
        } else {
            $reflection = new \ReflectionClass($type);
            $this->reflectionMap[$type] = $reflection;
            return $reflection;
        }
    }
}
...
class HessianFactory
{
    ...
    function getWriter($stream, $options)
    {
        ...
        if (!$options->objectFactory instanceof IHessianObjectFactory) {
            $options->objectFactory = new HessianObjectFactory();
        }
        return $writer;
    }
